...
void personal_details() {
	...
	char query[200];
	printf("****************************\n\n");
	sprintf(query, "SELECT Name FROM student where Id=\"%s\";", username);
	mysql_query(conn, query);
	res_set = mysql_store_result(conn);
	row = mysql_fetch_row(res_set);
	printf("Name:       %s\n\n", row[0]);
	...
	sprintf(query, "SELECT Address FROM student where Id=\"%s\";", username);
	mysql_query(conn, query);
	res_set = mysql_store_result(conn);
	row = mysql_fetch_row(res_set);
	printf("Address:       %s\n\n", row[0]);
	...
	sprintf(query, "SELECT MaximumPreferredEnrollment FROM student where Id=\"%s\";", username);
	mysql_query(conn, query);
	res_set = mysql_store_result(conn);
	row = mysql_fetch_row(res_set);
	...
	sprintf(query, "SELECT NonPreferredClassroomType FROM student where Id=\"%s\";", username);
	mysql_query(conn, query);
	res_set = mysql_store_result(conn);
	row = mysql_fetch_row(res_set);
	...
	while (1)
	{
		...
		else if (!strcmp(answer, "Y"))
		{
			...
			scanf("%d", &dis_num);
			sprintf(query, "CALL set_MaximumPreferredEnrollment(\"%s\",\"%d\");", username,dis_num);
			mysql_query(conn, query);
			mysql_close(conn);
			conn = mysql_init(NULL);
			mysql_real_connect(
	...
	while (1)
	{
		...
		else if (!strcmp(answer, "Y"))
		{
			...
			scanf("%s", &dis_type);
			sprintf(query, "CALL set_NonPreferredClassroomType(\"%s\",\"%s\");", username, dis_type);
			mysql_query(conn, query);
			mysql_close(conn);
			conn = mysql_init(NULL);
			mysql_real_connect(
